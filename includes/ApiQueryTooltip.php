...
class APIQueryTooltip extends APIBase {
		...
		if ( $options['cat'] ) {
			if ( $this->mConf->lateCategoryFiltering() ) {
				if ( $targetTitle !== null ) {
					...
					$finder->seed( [ $targetTitle->getArticleID() ], [ $category ] );
	...
	public function getAllowedParams() {
		return [ 'target' => [ NumericDef::PARAM_MAX => 'string' ],
				  'tooltip' => [ ParamValidator::PARAM_TYPE => 'string' ],
				  'options' => [ ParamValidator::PARAM_TYPE => 'string', ParamValidator::PARAM_REQUIRED => true ],
				];
	}
