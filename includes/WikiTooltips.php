...
class WikiTooltips {
	 ...
	 * @var array
	 */
	private static $mCategoryFilterLookup = [];
	 ...
	 * @var array
	 */
	private static $mCategoryFilterCache = [];
	 ...
	 * @var array
	 */
	private static $mImageLinkTargets = [];
	...
	private static function performDelayedInitialization(): void {
		...
		if ( self::$mLoadingTooltipHtml !== null ) {
			...
			} elseif ( self::$mConf->latePageTitleParse() &&
				  !self::$mConf->assumeNonemptyPageTitle() &&
				  self::$mEmptyPageNameTooltipHtml === null
				) {
				self::$mUseTwoRequestProcess = true;
			} elseif ( self::$mConf->lateExistsCheck() && self::$mMissingPageTooltipHtml === null ) {
				self::$mUseTwoRequestProcess = true;
			} elseif ( self::$mConf->lateCategoryFiltering() ) {
				self::$mUseTwoRequestProcess = true;
			}
		}
			...
			if ( self::passesCategoryFilter( $target ) ) {
				...
				if ( self::$mConf->earlyPageTitleParse() ) {
				  ...
				  // See https://github.com/SemanticMediaWiki/SemanticMediaWiki/issues/1181
					...
					if ( trim( $tooltipTitleText ) !== "" ) {
						...
						if ( $tooltipTitle !== null ) {
							if ( self::$mConf->earlyExistsCheck() ) {
								...
								} elseif ( self::$mMissingPageTooltipHtml !== null ) {
					...
					} elseif ( self::$mEmptyPageNameTooltipHtml !== null ) {
